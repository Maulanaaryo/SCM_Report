...
;WITH ConsolidatedCarrierDetails AS (
 SELECT
   ...
   SUM(CASE
         WHEN cd.[20] <> 0 THEN cd.CostQty * cd.[20]
         WHEN cd.[40] <> 0 THEN cd.CostQty * cd.[40]
         WHEN cd.[Other] <> 0 THEN cd.CostQty * cd.[Other]
         ELSE 0
       END) AS 'QTY' ,
   ...
   SUM(CASE
         WHEN cd.[20] <> 0 THEN cd.CostRate * cd.CostQty* cd.[20]
         WHEN cd.[40] <> 0 THEN cd.CostRate * cd.CostQty * cd.[40]
         WHEN cd.[Other] <> 0 THEN cd.CostRate * cd.CostQty * cd.[Other]
         ELSE 0
       END) AS 'Buy per CW (IDR)',
   ...
   SUM(CASE WHEN cd.RevCurrency = 'USD' THEN
         CASE
           ...
           ELSE 0
         END
       ELSE cd.RevAmount END) AS 'Sell per CW (IDR)',
   -- JIKA HANYA SELL PER CW x EXC SELL
   -- SUM(CASE WHEN cd.RevCurrency = 'USD' THEN cd.RevRate * cd.RevExchRate
   --     ELSE cd.RevAmount END) AS 'Sell per CW (IDR)',
...
-- SELECT *
-- FROM #CarrierDetails
-- WHERE RevRate <> 0 OR CostQty <> 0 OR [Other] <> 0;
